...
// // Request interceptor for adding authentication or other global headers
...
export const getActivityOverTime = (days = 30) => 
  fetchApi<ActivityData[]>(`/analytics/activity?days=${days}`)
export const getTopPages = (limit = 10) => 
  fetchApi<PageAnalytics[]>(`/analytics/pages?limit=${limit}`)
export const getStudentEngagement = () => 
  fetchApi<StudentEngagement[]>("/analytics/engagement")
export const getModuleEngagement = () => 
  fetchApi<ModuleEngagement[]>("/analytics/modules")
export const getTimeOfDayActivity = () => 
  fetchApi<TimeOfDayActivity[]>("/analytics/time-of-day")
